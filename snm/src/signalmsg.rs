use dbus_interface;
use dbus;
use connection_types::{ConnectionStatus, ConnectionInfo};
    ...
    pub fn log(&self) {
        match self {
            ...
            SignalMsg::ConnectStatusChanged(ref status) => {
                println!("Connect status changed to {}", match status {
            ...
            SignalMsg::StateChanged(ref info) => {
                match info {
                    ConnectionInfo::ConnectingEth |
                    ConnectionInfo::ConnectingWifi(_) => {
                        println!("Connecting")
                    }
    ...
    pub fn emit(self, connection: &dbus::Connection, path: &dbus::Path) {
        ...
        let dbus_message = match self {
                ...
                for i in networks {
                    match i {
                        NetworkInfo::Ethernet => {
                            arg_data.push((1, "Ethernet connection".to_string(),
                                           false, 100 as u32));
                        }

                        NetworkInfo::Wifi(essid, quality, enc, _) => {
                            arg_data.push((2, essid.to_string(),
                                           *enc, *quality));
                        }
                    }
                }
            ...
            SignalMsg::ConnectStatusChanged(s) => {
                let args = dbus_interface::ComGithubOkeriSnmConnectStatusChanged{status: s as u32};
                args.to_emit_message(path)
            ...
            SignalMsg::StateChanged(info) => {
                ...
                let args = dbus_interface::ComGithubOkeriSnmStateChanged {
                    state: arg_data,
                };
                args.to_emit_message(path)
            }
