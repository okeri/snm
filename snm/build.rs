...
fn main() {
    ...
    let output = File::create(gen_source_path)
        .expect(&format!("Can't create «{}»", gen_source_path.to_str().unwrap()));

    let input = File::open(interface_path)
        .expect(&format!("Can't open «{}»", interface_path.to_str().unwrap()));
